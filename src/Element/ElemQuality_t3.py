...
def ElemQuality_t3(x):
    """
     ...
     INPUT: x (3x2) - coordinates of element (triangle) vertices
     OUTPUT: d (1x1) - a measure of the distortion of a trinagle, d \in [0 .. 1]
    """
    # relative coordinates of element vertices
    x = np.arange(x[2, :] - x[1, :], x[3, :] - x[2, :], x[3, :] - x[1, :])
    # triangular element quality measure(Bank and Smith)
